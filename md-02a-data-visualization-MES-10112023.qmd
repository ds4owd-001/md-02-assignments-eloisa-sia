...
1.  Run the code chunk with the label 'load-packages' to load the required packages. Tip: Click on the green play button in the top right corner of the code chunk.
...
5.  What are the data types of the three variables used for aesthetic mappings?
    ...
    The data types for the variables used for aesthetic mappings are: continent -\> *category*, life expectancy -\> *continuous*.

### Boxplot for `gapminder_2007` [with outliers]{.underline}
...
### Boxplot for `gapminder_2007` [without outliers]{.underline}
...
Source for the code used to remove outliers was taken from: https://stackoverflow.com/questions/5677885/ignore-outliers-in-ggplot2-boxplot
...
### **Timeseries plot with `geom_line()`**
...
### **Timeseries plot with `geom_point()`**
...
## Task 8: Create a barplot with `geom_col()`
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? [**Write your text here**]{.underline}:

    -   When no changes where made to `geom_bar()`, it used `stat = "count"` by default, which involves providing the count of the number of cases for each `x = continent` position. Since this default function was not changed when `geom_bar()` was used in the code, it resulted in both `geom_col()` and `geom_bar()` plots to be identical.
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe:
    ...
    -   Overall, `seq()` allows for a numeric vector to set the bin bounderies.

    -   The first and second value used in `seq()` sets the range of the x-axis with the first value representing the minimum value and the second value representing the maximum value.

    -   The third value used in `seq()` sets the bin width.
...
4.  What is stored in the `country_colors` object? Find out by executing the object in the Console (type it to the Console and hit enter). Do the same again, but with a question mark `?country_colors`.
    ...
    -   The color schemes for 142 countries are stored in `country_colors`.
